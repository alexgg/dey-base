...
RUN apt-get update && apt-get install -y locales gawk wget git-core diffstat unzip texinfo gcc-multilib build-essential chrpath socat libsdl1.2-dev xterm cpio curl python dos2unix && apt-get clean && rm -rf /var/lib/apt/lists/* /tmp/* /var/tmp/*
...
ARG DEY_INSTALL_PATH="/usr/local/dey-1.6"
ARG DEY_BRANCH="daisy"
